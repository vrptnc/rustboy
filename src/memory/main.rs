...
impl<T> Memory for MainMemory<T> where T: Memory {
  ...
  fn write(&mut self, address: usize, value: u8) {
    match address {
      ...
      0xC000..=0xCFFF => self.ram.write(address, value),
      0xD000..=0xDFFF => self.ram_banks.write(address, value),
      0xE000..=0xFDFF => self.reserved_area_1.write(address, value),
      0xFE00..=0xFEBF => self.oam.write(address, value),
      0xFEA0..=0xFEFF => self.reserved_area_2.write(address, value),
      0xFF00..=0xFF7F => self.control_registers.write(address, value),
...
impl<T> MainMemory<T> where T: Memory {
  pub fn new(rom: T) -> MainMemory<T> {
    return MainMemory {
      rom,
      vram: BankMemory::<0x2000, 2>::new(0x8000),
      ram: LinearMemory::<0x1000>::new(0xC000), //Bank 0 of RAM
      ram_banks: BankMemory::<0x1000,7>::new(0xD000), // Seven banks of switchable 4k RAM, switched by SVBK register (0xFF70),
      reserved_area_1: LinearMemory::<0x1E00>::new(0xE000), // In theory, this area is prohibited, but let's map it anyway
      oam: LinearMemory::<0xA0>::new(0xFE00),
      reserved_area_2: LinearMemory::<0x60>::new(0xFEA0), // In theory, this area is prohibited, but let's map it anyway
      control_registers: LinearMemory::<0x80>::new(0xFF00),
      stack: LinearMemory::<127>::new(0xFF80),
      interrupt_enable: 0
    };
  }
}